...
class ContextSpec extends WordSpec with MustMatchers {
  ...
  "A Context" must {
    ...
    "instantiate a complex object" in {
      val ast = Parser.parse("configure TCwithConstructor complex construct 12 with Value 42 call method end").right.get
    ...
    "instantiate a configurable object" in {
      val ast = Parser.parse("configure TCConfigurable conf with arg hallo with Value 12 end").right.get
      val result = ctx load ast
    ...
    "instantiate a nested object" in {
      val ast = Parser.parse("configure TCNested nest construct configure TCSimple end end").right.get
      val result = ctx load ast
      result must be (ctx.copy(dict = Map("nest" -> TCNested(TCSimple()))))
    }

    "resolve references" in {
      val ast = Parser.parse("configure TCSimple simple end configure TCNested nest construct reference simple end").right.get
      val result = ctx load ast
      result must be (ctx.copy(dict = Map("simple" -> TCSimple(), "nest" -> TCNested(TCSimple()))))
    }

    "fail when classes mismatch" in {
      val ast = Parser.parse("configure TCConfigurable conf end configure TCNested nest construct reference conf end").right.get
      val result = ctx load ast
